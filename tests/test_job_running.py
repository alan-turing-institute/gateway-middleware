...
def test_start_job_without_values(demo_app, test_job_id_no_values):
    ...
    with Mocker() as m:
        start_url = "{}/{}/start".format(
            demo_app.config["JOB_MANAGER_URL"], test_job_id_no_values
        )
    ...
    assert Job.query.get(test_job_id_no_values).status == JobStatus.NOT_STARTED.value
...
def test_start_job(demo_app, test_job_id):
    ...
    """
    with Mocker() as m:
        start_url = f"{demo_app.config['JOB_MANAGER_URL']}/{test_job_id}/start"
        m.post(start_url, json={"messages": ["foo"]})
