...
class ParamSpecSchema(ma.ModelSchema):
    ...
    def make_param_spec(self, data):
        ...
        return ParameterSpec(
            id=data.get("id"), name=data.get("name"), value=data.get("value")
        )
...
class CaseFieldSchema(ma.ModelSchema):
    ...
    def make_case_field(self, data):
        ...
        """
        casefield = CaseField(
            name=data.get("name"), case_field_id=data.get("case_field_id")
        )
        if data.get("specs"):
...
class CaseSchema(ma.ModelSchema):
    ...
    def make_case(self, data):
        ...
        Make an instance from a map
        """
        case = Case(cid=data.get("id"), name=data.get("name"))
        if data.get("fields"):
            cfs = CaseFieldSchema()
...
class CaseHeaderSchema(ma.ModelSchema):
    ...
    links = ma.Hyperlinks({"self": ma.URLFor("caseapi", case_id="<id>")})
...
class JobHeaderSchema(ma.ModelSchema):
    ...
    class Meta:
        ...
        model = Job
        fields = ("id", "name", "status", "user", "links", "description", "parent_case")
